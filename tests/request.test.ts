import { HTTPRequest, NodeHoyoError, LanguageEnum } from '../src'
...
test('send() should throw NodeHoyoError', async (t) => {
...
test('send() should throw NodeHoyoError when JSON response is invalid', async (t) => {
  nock('https://example.com')
    .get('/')
    .query(true)
    .reply(() => {
