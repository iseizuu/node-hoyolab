...
export class HTTPRequest {
   ...
   * @returns A Promise that resolves with the response data, or rejects with a NodeHoyoError if an error occurs.
   * @throws {NodeHoyoError} if an error occurs rejects with a NodeHoyoError
   */
  async send(
    url: string,
    method: 'GET' | 'POST' = 'GET',
      ...
      return new Promise<HTTPServerResponse>((resolve, reject) => {
        ...
        const client = request(hostname, options, (res: IncomingMessage) => {
          if (res.statusCode === 429) {
            ...
            // If the status code is between 400 and 599 (inclusive), reject the promise with an NodeHoyoError
            reject(
              new NodeHoyoError(
          ...
          res.on('end', () => {
            ...
            if (res.headers['content-type'] === 'application/json') {
              try {
                ...
                reject(
                  new NodeHoyoError('Failed to parse response body as JSON'),
                )
              }
            } else {
              reject(
          ...
          res.on('error', (err: Error) => {
            /* c8 ignore next */
            reject(new NodeHoyoError(err.message))
          })
        })
