...
export class GenshinDiaryModule {
   ...
   * @throws {@link NodeHoyoError} when the uid or region parameter is missing or invalid
   * @remarks
  ...
  async list(
    ...
    if (Object.values(DiaryMonthEnum).includes(month) === false) {
      throw new NodeHoyoError('The given month parameter is invalid !')
    }
   ...
   * @throws {@link NodeHoyoError} when the uid or region parameter is missing or invalid, or when the type or month parameter is invalid
   * @remarks
   * This method sends a request to the Genshin Impact API to get the daily note information for a user.
   * The user's region and UID must be set before calling this method, otherwise an error will be thrown.
   */
  async detail(
    ...
    if (Object.values(DiaryEnum).includes(type) === false) {
      throw new NodeHoyoError('The given type parameter is invalid !')
    }
